...
(async() => {
  ...
  let masterKey = await HDWallet.exportMasterPrivateKey(answers.passphrase, answers.password, answers.network);
  if (!masterKey)
    console.log(' - Can not create master key');
  else {
    console.log(' - Master private key (HEX):', masterKey.masterPrivateKey);
    console.log(' - Master private key (WIF):', masterKey.masterPrivateKeyWif);
    console.log(' - Master private key (WIF-Uncompress):', masterKey.masterPrivateKeyWifUncompress);
